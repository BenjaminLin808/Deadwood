    ...
    public void setUp(Map<String, Location> locations){
        ...
        for (String key : keys) {
            ...
            if(!key.equals("office") && !key.equals("trailer")){
                int x = board.getLocation(key).getCoordinates().getX();
                int y = board.getLocation(key).getCoordinates().getY();
                int h = board.getLocation(key).getCoordinates().getHeight();
                int w = board.getLocation(key).getCoordinates().getWidth();
